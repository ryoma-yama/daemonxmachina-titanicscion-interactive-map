...
const PRIMARY_CATEGORY = ALL_CATEGORIES.includes("music")
        ? "music"
        : ALL_CATEGORIES[0];
const PRIMARY_DESERT_MARKER = categoryToMarkers
        .get(PRIMARY_CATEGORY)
        .find((marker) => marker.mapId === "desert") || categoryToMarkers.get(PRIMARY_CATEGORY)[0];
const SECONDARY_MAP_ID = PRIMARY_DESERT_MARKER.mapId === "forest" ? "mountains" : "forest";
const SECONDARY_MARKER = categoryToMarkers
        .get(PRIMARY_CATEGORY)
        .find((marker) => marker.mapId === SECONDARY_MAP_ID);
const CARD_MARKER = categoryToMarkers.get("card")?.find((marker) => marker.mapId === "desert");
...
test.describe("filter pane", () => {
        ...
        test("toggling a single category hides related markers and results", async ({ page }) => {
                test.skip(!PRIMARY_DESERT_MARKER, "No marker available for primary category");
                ...
                await expect(page.locator("#search-message")).toContainText("No markers found");
        });

        test("all button re-enables every category", async ({ page }) => {
                await openFilterPane(page);
                const toggles = [PRIMARY_CATEGORY, CARD_MARKER?.id ? "card" : ALL_CATEGORIES[1]];
        ...
        test("preferences persist after reload", async ({ page }) => {
                ...
                await expect(page.locator(`[data-marker-id="${PRIMARY_DESERT_MARKER.id}"]`)).toHaveCount(0);
        ...
        test("filter selection is shared across maps", async ({ page }) => {
                test.skip(!SECONDARY_MARKER, "No secondary marker available for selected category");
                await openFilterPane(page);
                await page.getByTestId(`filter-item-${PRIMARY_CATEGORY}`).click();
        ...
        test("filter changed event emits payload", async ({ page }) => {
                ...
                expect(Array.isArray(events[events.length - 1].selectedCategories)).toBe(true);
        });
