...
const createTempDir = () => fs.mkdtempSync(path.join(os.tmpdir(), 'add-marker-'));
...
describe('parseArguments', () => {
  it('parses positional arguments with optional description', () => {
    const args = parseArguments(['forest', '100', '200', 'card', 'Marker', 'Detailed description']);
  ...
  it('supports --dry-run flag', () => {
    const args = parseArguments(['--dry-run', 'forest', '10', '20', 'card', 'Name']);
  ...
  it('throws when required arguments are missing', () => {
    expect(() => parseArguments(['forest'])).toThrow('Missing required arguments');
  });

  it('throws when coordinates are not numbers', () => {
    expect(() => parseArguments(['forest', 'a', '100', 'card', 'Name'])).toThrow('Invalid x coordinate');
    expect(() => parseArguments(['forest', '100', 'b', 'card', 'Name'])).toThrow('Invalid y coordinate');
  });
});
...
describe('validateInputs', () => {
  ...
  it('rejects non-number coordinates', () => {
    expect(() => validateInputs({ x: '10', y: 10, category: 'card', name: 'Name' })).toThrow('Coordinates must be numbers');
  });

  it('rejects negative coordinates', () => {
    expect(() => validateInputs({ x: -1, y: 0, category: 'card', name: 'Name' })).toThrow('Coordinates must be non-negative');
  });

  it('rejects invalid categories', () => {
    expect(() => validateInputs({ x: 0, y: 0, category: 'invalid', name: 'Name' })).toThrow('Invalid category');
  });

  it('rejects non-string names', () => {
    expect(() => validateInputs({ x: 0, y: 0, category: 'card', name: null })).toThrow('Marker name cannot be empty');
  });

  it('rejects empty names', () => {
    expect(() => validateInputs({ x: 0, y: 0, category: 'card', name: '   ' })).toThrow('Marker name cannot be empty');
  });

  it('normalizes non-string descriptions to empty string', () => {
    const validated = validateInputs({ x: 0, y: 0, category: 'card', name: 'Name', description: null });
...
describe('loadGeojson', () => {
  it('returns empty structure when file does not exist', () => {
    ...
    expect(loadGeojson(tempPath)).toEqual({ type: 'FeatureCollection', features: [] });
  });
  ...
  it('throws when GeoJSON type is incorrect', () => {
    ...
    expect(() => loadGeojson(filePath)).toThrow('File is not a valid GeoJSON FeatureCollection');
  });
  ...
  it('throws when features is not an array', () => {
    ...
    expect(() => loadGeojson(filePath)).toThrow("Invalid GeoJSON: 'features' must be an array");
  });
...
describe('saveGeojson', () => {
  it('writes file with trailing newline', () => {
    ...
    expect(fs.readFileSync(filePath, 'utf-8')).toBe(`${JSON.stringify(data, null, 2)}\n`);
  });
...
describe('extractExistingData', () => {
  it('collects ids and coordinates', () => {
    const data = {
      ...
      features: [
        { properties: { id: 'forest-001', name: 'A' }, geometry: { coordinates: [100, 200] } },
...
describe('generateNextId', () => {
  it('generates sequential ids with padding', () => {
    ...
    expect(generateNextId('forest', new Set(['forest-001', 'forest-002']))).toBe('forest-003');
  });
...
describe('createMarkerFeature', () => {
  ...
  it('adds description when provided', () => {
    const feature = createMarkerFeature('forest-002', 'Name', 'card', 1, 2, 'Desc');
...
describe('addMarkerToGeojson', () => {
  it('initializes missing feature array', () => {
    const result = addMarkerToGeojson({}, createMarkerFeature('forest-002', 'Name', 'card', 1, 2));
    expect(result.features).toHaveLength(1);
  });

  it('keeps markers sorted by id', () => {
    const data = { features: [createMarkerFeature('forest-010', 'A', 'card', 1, 1)] };
    addMarkerToGeojson(data, createMarkerFeature('forest-002', 'B', 'card', 2, 2));
    expect(data.features[0].properties.id).toBe('forest-002');
  });

  it('handles items without ids during sorting', () => {
    const data = { features: [{}] };
    addMarkerToGeojson(data, createMarkerFeature('forest-002', 'Name', 'card', 1, 1));
    expect(data.features).toHaveLength(2);
    expect(data.features.some((feature) => feature?.properties?.id === 'forest-002')).toBe(true);
  });

  it('supports markers without ids', () => {
    const data = { features: [createMarkerFeature('forest-001', 'Existing', 'card', 0, 0)] };
    const anonymousFeature = { type: 'Feature', geometry: { type: 'Point', coordinates: [1, 1] }, properties: {} };
    addMarkerToGeojson(data, anonymousFeature);
    expect(data.features).toHaveLength(2);
    expect(data.features.some((feature) => feature.properties?.id === undefined)).toBe(true);
  });
});
...
describe('displayMarkerPreview', () => {
  it('logs marker preview including optional description', () => {
    const log = vi.spyOn(console, 'log').mockImplementation(() => {});
    const feature = createMarkerFeature('forest-001', 'Name', 'card', 1, 2, 'Desc');
...
describe('main', () => {
  ...
  it('prints preview on dry run', () => {
    const markersDir = createTempDir();
    const { log } = captureLogs();
    main(['--dry-run', 'forest', '10', '20', 'card', 'Name', ' Description '], markersDir);
