...
export class FilterPane {
        constructor({ filterManager }) {
                ...
                if (!this.container || !this.toggleButton || !this.listElement || !this.allButton || !this.noneButton) {
        ...
        hasRenderedCategories(categories) {
                ...
                return this.renderedCategories.every((category, index) => category === categories[index]);
        }
        ...
        handleRowKeydown(event, category) {
                ...
                const currentIndex = this.rowOrder.findIndex((row) => row.dataset.category === category);
                if (currentIndex === -1) {
                        return;
                }
                ...
                if (event.key === "ArrowDown") {
                        ...
                        event.preventDefault();
                        const prevIndex = (currentIndex - 1 + this.rowOrder.length) % this.rowOrder.length;
                        this.rowOrder[currentIndex].setAttribute("tabindex", "-1");
        ...
        updateCategoryStates(selectedCategories) {
                ...
                this.categoryElements.forEach((elements, category) => {
                        ...
                        elements.item.classList.toggle("filter-pane__item--disabled", !isSelected);
                        elements.label.classList.toggle("filter-pane__label--disabled", !isSelected);
                });
        }
