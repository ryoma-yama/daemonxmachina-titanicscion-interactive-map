...
export class MapView {
  ...
  loadMap(mapDefinition, mapId) {
    ...
    // Add image overlay
    this.currentImageOverlay = L.imageOverlay(mapDefinition.imagePath, mapDefinition.bounds);
  ...
  loadMarkers(markersPath, collectionState) {
    fetch(markersPath)
      ...
      .then(data => {
        ...
        console.log(`Loaded ${validFeatures.length} valid features out of ${data.features.length} total for ${this.currentMapId}`);
        ...
        this.currentMarkerLayer = L.geoJSON(validData, {
          ...
          onEachFeature: (feature, layer) => {
            const popupContent = this.createPopupContent(feature, collectionState);
        ...
        console.log(`GeoJSON markers loaded successfully for ${this.currentMapId}`);
      })
      .catch(error => {
  ...
  updateMarkerState(markerId, isCollected) {
    ...
    if (marker) {
      ...
      const markerSize = this.getMarkerSize();
      const newIcon = createCategoryIcon(feature.properties.category, markerSize, isCollected);
